...
import {ArrowDown, RefreshLeft, Tools} from '@element-plus/icons-vue'
import {ref, onMounted} from 'vue'
import {pageFuzzyUsingGet} from '@/openapi/api/fraudCaseController'
...
const getCases = async () => {
  ...
  const res = await pageFuzzyUsingGet(pageRequestDTO.value)
...
<template>
  ...
  <div class="case-table-container" v-if="caseList.length > 0">
    ...
    <div class="table-section">
      ...
      <el-table :data="caseList" style="width: 100%"  stripe="true">
        ...
        <el-table-column prop="date" label="日期" width="150">
          <template #default="scope">
            {{formatRelativeTime(scope.row.date)}}
          </template>
        </el-table-column>
  ...
  <!-- 文章分页 -->
  <empty-item v-if="caseList.length === 0"></empty-item>
